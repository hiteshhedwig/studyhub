  ...
  return (
    <>
      ...
      <section className="grid two" style={{ marginTop: 20 }}>
        <div className="card" style={{ minHeight: 320 }}><h2>Daily study time</h2>{series.some((item) => item.minutes) ? <StudyTimeChart data={series} /> : <EmptyState>Charts will fill in after completed Pomodoros.</EmptyState>}</div>
        ...
        <div className="card"><h2>Time by topic</h2><div className="list">{topics.map((topic) => <div className="list-item" key={topic.id}><div className="split"><span>{topic.title}</span><span>{sessions.filter((session) => session.topic_id === topic.id).reduce((sum, session) => sum + session.focus_minutes * session.pomodoros_completed, 0)}m</span></div></div>)}</div></div>
      </section>
    </>
  );
}
...
function TopicList({ items }: { items: { id: string; title: string; mastery_score: number }[] }) {
  return items.length ? <div className="list">{items.map((topic) => <div className="list-item" key={topic.id}><div className="split"><span>{topic.title}</span><span className="pill">{topic.mastery_score}%</span></div></div>)}</div> : <EmptyState>No topics yet.</EmptyState>;
}
